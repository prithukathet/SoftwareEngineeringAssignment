     ...
     public static void main(String[] args) {
        ...
        while(!validInput) {
            ...
            try { // Try catch encase the user input invalid characters
                ...
                if (k >= 10 && k < Math.pow(2, 32)) { // TODO: Upper limit on K so you don't run out memory
                    validInput = true;
                } else {
     ...
     public static void calculateNearMiss(int n, int k) { 
         ...
         for (int x = 10; x <= k; x++) {
            for (int y = 10; y <= k; y++) {
                 ...
                 int z = (int) Math.pow(xnPlusYn, 1.0 / n); // Start z at the approximate nth root of (x^n + y^n)
